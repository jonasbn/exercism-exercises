...
const ANCIENT_KEYWORD: &str = "nail";
...
pub fn build_proverb(list: &[&str]) -> String {
    ...
    if !list.is_empty() {
        for &word in list.iter() {
            ...
            if word != ANCIENT_KEYWORD && word != MODERN_KEYWORD {
                verses.push_str(&regular_verse(verse.to_string(), word.to_string()));
            }
        }
        if modern_period {
            verses.push_str(&end_verse("pin".to_string()));
        } else {
            verses.push_str(&end_verse("nail".to_string()));
        }
    }
